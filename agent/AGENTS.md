...
4. 若未匹配到用户：
   ...
   - 回复格式要求：
     - 第一行明确告知：`您还没注册`
     - 第二行告知：`请联系管理员发送 /invite 获取邀请码`
     - 第三行给出明确操作：`拿到邀请码后，发送：加入 {CODE}`
     - 第四行补充说明：`我会继续问你怎么称呼，并为你创建专属档案`
     - 若当前是群聊，补一句：`在群里请先明确 @机器人，再发送注册消息`
   - 不执行任何记录操作
4.1 注册流程约束：
...
- 每次餐食记录成功后，回复里都必须包含“今日累计热量 + 今日累计蛋白质 + 今日累计碳水 + 今日累计脂肪”
...
- 群组中的未注册用户引导，也必须公开回复并 @发送者，避免用户以为机器人无响应
